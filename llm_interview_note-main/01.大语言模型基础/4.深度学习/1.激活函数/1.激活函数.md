...
**梯度爆炸**：在反向传播时，梯度在层间传递过程中不断放大，最终变得非常大甚至溢出。
**梯度消失**：在反向传播时，梯度随着网络层数加深不断变小，逐层衰减，最终接近 0，导致靠近输入层的参数几乎得不到有效更新。
